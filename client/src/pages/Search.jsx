...
const Search = () => {
 ...
 const minPrice = formState.minPrice || "";
      const maxPrice = formState.maxPrice || "";

      const res = await fetch(
        `/api/posts?searchTerm=${searchTermState}&type=${formState.type}&parking=${formState.parking}&furnished=${formState.furnished}&offer=${formState.offer}&minPrice=${minPrice}&maxPrice=${maxPrice}&page=${pageCount}`
      );
        const json = await res.json();
        if (json.success === false) {
          setLoading(false);
                    ...
                    <div className="feilds max-w-xs">
                      ...
                      <div className="price_filter mt-4">
                        ...
                        <div className="flex flex-row gap-4 mt-1">
                          <input
                            className="flex-1 h-8 w-24 px-3 border rounded-sm border-gray-300 focus:border-brand-blue focus:outline-none"
                            type="number"
                            name="minPrice"
